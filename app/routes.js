...
router.post('/pip-answer', function(request, response) {

    var pip = request.session.data['pip']
    if (pip == "yes"){
        response.redirect("/core/page-pre-dwp-kbv-transition")
    } else {
        response.redirect("/core/page-different-security-questions")
    }
})
